...
export default class PickerParameter extends Component<IProps, IStates> {
    ...
    _move(dy) {
        ...
        this.middleHeight = Math.abs(-index * h + dy);
    ...
    _handlePanResponderMove(evt, gestureState) {
        ...
        // turn down
        if (dy > 0) {
            this._move(dy > this.index * h ? this.index * h : dy);
        } else {
            this._move(dy < (this.index - this.state.items.length + 1) * h ? (this.index - this.state.items.length + 1) * h : dy);
        }
    }

    _handlePanResponderRelease(evt, gestureState) {
        let middleHeight = this.middleHeight;
        this.index = middleHeight % h >= 20 ? Math.ceil(middleHeight / h) : Math.floor(middleHeight / h);
        this._move(0);
        this._onValueChange();
    }
